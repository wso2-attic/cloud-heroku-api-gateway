...
UNZIPPED_FILE_NAME=wso2am-cloud-micro-gw-2.6.0
DOWNLOAD_ZIP_FILE_NAME=wso2am-cloud-micro-gw-2.6.0.zip
ON_PREM_GATEWAY_DOWNLOAD_LINK=https://s3.amazonaws.com/wso2cloud-resources/on-premise-gateway/wso2am-cloud-micro-gw-2.6.0.zip
...
if [ ! -f $DOWNLOAD_ZIP_FILE_NAME ]
    then
        ...
        if [ ! -d $UNZIPPED_FILE_NAME ]; then
            ...
            sed -i 's/JVM_MEM_OPTS="-Xms256m -Xmx1024m"/JVM_MEM_OPTS="-Xms256m -Xmx1600m"/' $UNZIPPED_FILE_NAME/bin/wso2server.sh
            sed -i 's#AUTOSTART="${WSO2_CLOUD_AUTOSTART:-"false"}"#AUTOSTART="${WSO2_CLOUD_AUTOSTART:-"true"}"#' $UNZIPPED_FILE_NAME/bin/cloud-init.sh
            sed -i 's/<startIndexing>true<\/startIndexing>/<startIndexing>false<\/startIndexing>/' $UNZIPPED_FILE_NAME/repository/conf/registry.xml
	    sed -i "s/8280/$PORT/" $UNZIPPED_FILE_NAME/repository/conf/axis2/axis2.xml
        fi
fi
